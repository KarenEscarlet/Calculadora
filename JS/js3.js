document.getElementById("calcular").addEventListener("click", function() {
    ...
    if (isNaN(capital) || isNaN(interes) || isNaN(plazo) || capital <= 0 || interes <= 0 || plazo <= 0) {
        mostrarError("Ingresa valores válidos en todos los campos.");
        return;
    } else {
        ocultarError();
    }
    ...
    resultadoElement.innerHTML = "La cuota mensual del préstamo es: <strong>$" + cuota.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "</strong><br>" +
                                "El total de intereses pagados es: <strong>$" + totalInteresPagado.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "</strong>";
